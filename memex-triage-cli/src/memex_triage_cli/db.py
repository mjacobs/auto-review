...
Adapted verbatim from memex-sync/src/memex_sync/db.py (repo convention is
independent siblings, no shared library). The helper is role-agnostic — it
connects as whatever role the DSN names; here that is the `memex_triage` role
via PG_DSN (alias MEMEX_TRIAGE_PG_DSN). An omitted password is resolved from a
repo-local `.pgpass` or `~/.pgpass` (libpq format); explicit DSN passwords win.
"""
...
def connect() -> Iterator[psycopg.Connection]:
    ...
    exception — the triage flips lean on that for their one-transaction shape.
    """
