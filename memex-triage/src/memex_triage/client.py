...
created_at. No vault writes here.
"""
...
def server_head(
    *,
    ...
    most recently inserted and therefore carries the max seq. Used to bootstrap
    a fresh watermark "at head". Under-estimating here is safe (a few extra
    historical items get delivered); only over-estimating could skip a capture.
    """
    settings = settings or get_settings()
    base = settings.memex_url.rstrip("/")
    owned_client = client is None
