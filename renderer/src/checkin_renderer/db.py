...
independent siblings, no shared library; the pgpass fallback is lifted
verbatim per DESIGN.md). Connects as the `checkin_renderer` role via
CHECKIN_RENDERER_PG_DSN; an omitted password is resolved from a repo-local
`.pgpass` or `~/.pgpass` (libpq format), explicit DSN passwords win.
"""
...
def _dsn_with_pgpass_password(dsn: str) -> str:
    ...
    ``CHECKIN_RENDERER_PG_DSN`` always win.
    """
